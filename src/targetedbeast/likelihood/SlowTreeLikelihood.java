...
public class SlowTreeLikelihood extends GenericTreeLikelihood {
    ...
    final public Input<String> implementationInput = new Input<>("implementation", "name of class that implements this treelikelihood potentially more efficiently. "
    		...
    		+ "When multi-threading, multiple objects can be created.", SlowBeerLikelihoodCore.class.getName());
	...
	protected LikelihoodCore createLikelihoodCore(int stateCount) {
		// if (stateCount == 4) {
		// 	return new SlowBeerLikelihoodCore4();
		// } else {
			return new SlowBeerLikelihoodCore(stateCount);
		// }
    }
     ...
     */
    public void setPartials(Node node, int patternCount) {
        if (node.isLeaf()) {
