 ...
 */
public class SlowBeerLikelihoodCore extends LikelihoodCore {
    ...
    public SlowBeerLikelihoodCore(int nrOfStates) {
     ...
     */
    public void calculateStatesStatesPruning(int[] stateIndex1, double[] matrices1,
     ...
     */
    public void calculateStatesPartialsPruning(int[] stateIndex1, double[] matrices1,
	...
	public void calculateIntegratePartials(double[] inPartials, double[] outPartials) {
        int u = 0;
        int v = 0;
        for (int k = 0; k < nrOfPatterns; k++) {
            for (int i = 0; i < nrOfStates; i++) {
                outPartials[u] = inPartials[v];
                u++;
                v++;
            }
        }
    ...
    public void getNodePartials(int nodeIndex, double[] partialsOut) {
        System.arraycopy(partials[0][nodeIndex], 0, partialsOut, 0, partialsOut.length);
    }
	...
	public void setNodeMatrix(int nodeIndex, int matrixIndex, double[] matrix) {
        System.arraycopy(matrix, 0, matrices[0][nodeIndex],
	...
	public void getNodeMatrix(int nodeIndex, int matrixIndex, double[] matrix) {
        System.arraycopy(matrices[0][nodeIndex],
                matrixIndex * matrixSize, matrix, 0, matrixSize);
    }

	public double[] getNodeMatrices(int nodeIndex, boolean copy) {
        if (copy)
            return matrices[0][nodeIndex].clone();
        else
            return matrices[0][nodeIndex];
    }

	public double[] getNodeMatrices(int nodeIndex) {
        return getNodeMatrices(nodeIndex, false);
    }

    public double[] getNodeMatrices(Node node) {
        return getNodeMatrices(node.getNr());
        // return matrices[0][node.getNr()];
    }
        ...
        if (states[nodeIndex1] != null) {
            if (states[nodeIndex2] != null) {
                calculateStatesStatesPruning(
                        states[nodeIndex1], matrices[0][nodeIndex1],
                        states[nodeIndex2], matrices[0][nodeIndex2],
                        partials[0][nodeIndex3]);
            } else {
                calculateStatesPartialsPruning(states[nodeIndex1], matrices[0][nodeIndex1],
                        partials[0][nodeIndex2], matrices[0][nodeIndex2],
                        partials[0][nodeIndex3]);
            }
        } else {
            if (states[nodeIndex2] != null) {
                calculateStatesPartialsPruning(states[nodeIndex2], matrices[0][nodeIndex2],
                        partials[0][nodeIndex1], matrices[0][nodeIndex1],
                        partials[0][nodeIndex3]);
            } else {
                calculatePartialsPartialsPruning(partials[0][nodeIndex1], matrices[0][nodeIndex1],
                        partials[0][nodeIndex2], matrices[0][nodeIndex2],
                        partials[0][nodeIndex3]);
            }
        }
        ...
        if (states[nodeIndex1] != null) {
            if (states[nodeIndex2] != null) {
                calculateStatesStatesPruning(
                        states[nodeIndex1], matrices[0][nodeIndex1],
                        states[nodeIndex2], matrices[0][nodeIndex2],
                        partials[0][nodeIndex3], matrixMap);
            } else {
	...
	public void integratePartials(int nodeIndex, double[] proportions, double[] outPartials) {
        calculateIntegratePartials(partials[0][nodeIndex], proportions, outPartials);
    }
    ...
    protected void scalePartials(int nodeIndex) {
        ...
        for (int i = 0; i < nrOfPatterns; i++) {
            ...
            for (int k = 0; k < nrOfMatrices; k++) {
                for (int j = 0; j < nrOfStates; j++) {
                    if (partials[0][nodeIndex][v] > scaleFactor) {
                        scaleFactor = partials[0][nodeIndex][v];
                    }
                    v++;
                }
            ...
            if (scaleFactor < scalingThreshold) {
                ...
                for (int k = 0; k < nrOfMatrices; k++) {
                    for (int j = 0; j < nrOfStates; j++) {
                        partials[0][nodeIndex][v] /= scaleFactor;
                        v++;
                    }
                    v += (nrOfPatterns - 1) * nrOfStates;
                }
                scalingFactors[0][nodeIndex][i] = Math.log(scaleFactor);

            } else {
                scalingFactors[0][nodeIndex][i] = 0.0;
            }
            u += nrOfStates;
    ...
    public void getPartials(int nodeIndex, double[] outPartials) {
        double[] partials1 = partials[0][nodeIndex];
    ...
    /**
     * return the size of node partials
     */
